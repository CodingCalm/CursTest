...
export function RelatedPropositions({ postId }: RelatedPropositionsProps): React.JSX.Element {
  ...
  return (
    <ExpandableSection title="Relaterade förslag" count={relatedPropositions.length}>
      <div className="space-y-3">
        {relatedPropositions.map((proposition) => (
          <RelatedPropositionCard key={proposition.id} proposition={proposition} />
        ))}
      </div>
    </ExpandableSection>
  );
...
function RelatedPropositionCard({ proposition }: { proposition: any }): React.JSX.Element {
  return (
    <Link
