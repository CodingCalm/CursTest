...
export function VoteButton({
  ...
  const { initializePost, votePost, getUserVote, getUpvotes } =
    useVotingStore();
  ...
  const buttonClasses = `p-1 rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-400 focus-visible:ring-opacity-50 transition-colors duration-200 ${
    isActive
      ? isUpvote
        ? 'text-green-600'
        : 'text-red-600'
  ...
  return (
    <AuthenticatedButton
      ...
      onAuthenticatedAction={handleVote}
      actionName='rösta'
      aria-label={`Rösta ${isUpvote ? 'upp' : 'ner'}. Just nu ${currentUpvotes} röster`}
