...
export function VotingSection({
  ...
  return (
    <div
      className='flex flex-row items-center justify-between mt-4 pt-4 border-t border-gray-200'
      role='group'
      aria-label='Röstningssektion'
    >
      <div className='flex items-center gap-2'>
        <VoteButton direction='up' postId={postId} initialUpvotes={upvotes} />
        <span
          className='text-sm font-bold text-gray-900 mx-2'
          aria-live='polite'
        >
          {displayUpvotes}
        </span>
        <VoteButton direction='down' postId={postId} initialUpvotes={upvotes} />
      </div>
      <div className='flex items-center gap-2'>
        {showComments && <CommentDisplay comments={comments} />}
        <NominationButton
